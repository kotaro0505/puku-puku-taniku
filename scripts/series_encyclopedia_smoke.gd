...
func _ready()->void:
	...
	var base:Dictionary=game._series_entry("base");assert(game._is_series_unlocked(base));assert(str(base.get("display_name",""))=="原種図鑑" and game._series_species_entries("base").size()==21 and game.catalog_species.size()==124)
