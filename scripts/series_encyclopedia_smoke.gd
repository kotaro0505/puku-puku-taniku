...
func _ready()->void:
	...
	var base:Dictionary=game._series_entry("base");assert(game._is_series_unlocked(base));assert(game._series_species_entries("base").size()==21 and game.catalog_species.size()==114)
	...
	for future_id in expected_ids.slice(1):
		...
		if str(future_id) in ["metal","jewel","jelly","sweets","stone","yumekawa","forest_amber"]:assert(future_entry.species_ids.size()==10 and not game._can_browse_series(future_entry) and bool(future_entry.get("preview_catalog_when_locked",false)))
