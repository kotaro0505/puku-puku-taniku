...
func _ready()->void:
	...
	game.arrangement_ui._request_seed_purchase("normal");assert(game.puku_points==0 and game.normal_seed_bags==bags_at_seed_shop+3 and "3袋購入しました" in game.arrangement_ui.seed_shop_message.text)
	...
	game.formal_play_count=0;game.puku_points=10;game._open_shop_catalog_category();assert(game.arrangement_ui.visible and game.arrangement_ui.catalog_shop_page.visible and game.arrangement_ui.return_context=="shop" and game.arrangement_ui.catalog_shop_grid.get_child_count()==11)
	assert(game.arrangement_ui.catalog_shop_page.position==game.arrangement_ui.shop_page.position and game.arrangement_ui.catalog_shop_grid.custom_minimum_size==game.arrangement_ui.shop_grid.custom_minimum_size and game.arrangement_ui.catalog_shop_grid.get_parent().scroll_deadzone==12 and game.arrangement_ui._series_preview_texture(game._series_entry("sweets")).resource_path=="res://assets/catalog/sweets/sweets-strawberry-shortcake.png")
	game.arrangement_ui._request_catalog_purchase("metal");assert(bool(game.unlocked_series.get("metal",false)) and game.puku_points==5 and game.arrangement_ui.catalog_shop_grid.get_child_count()==11)
	...
	game._save();game.intro_story_complete=false;game.habitat_unlocked=false;game.puku_gauge_intro_complete=false;game.total_play_count=0;game._load_save();game._update_play_ui();assert(game._tutorial_fully_complete() and game.shop_button.visible)
