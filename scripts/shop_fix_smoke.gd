...
func _ready()->void:
	...
	game.shop_overlay.visible=true;game._prepare_shop_visit(true);game._on_armadillo_tapped();game._hide_shop_chatter(true);game._on_armadillo_tapped();game._hide_shop_chatter(true);game._on_armadillo_tapped()
	...
	game.formal_play_count=2;game.volume_seed_unlocked=false;game.premium_seed_unlocked=false;game._select_shop_product("volume")
	assert(game.shop_bag_label.text=="たね袋を1袋ずつ購入できます" and "あと1回プレイで解禁" in game.shop_product_detail_label.text and "36粒 / 袋" in game.shop_product_detail_label.text)
	game._select_shop_product("premium");assert("あと11回プレイで解禁" in game.shop_product_detail_label.text and "24粒 / 袋" in game.shop_product_detail_label.text and game.shop_product_detail_label.get_theme_font_size("font_size")>=15)
