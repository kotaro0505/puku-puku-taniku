...
func _ready()->void:
	...
	assert(game.shop_transfer_notice.visible and game.shop_transfer_notice.text=="渡した謎のたね ×13個" and game.shop_transfer_notice.get_theme_font_size("font_size")>=23)
	game._hide_shop_chatter(true);assert(not game.shop_transfer_notice.visible)
	game.formal_play_count=3;game.volume_seed_unlocked=false;game.premium_seed_unlocked=false;game._update_shop_ui()
	assert(game.shop_bag_label.text=="たね袋を1袋ずつ購入できます" and "あと4回プレイで解禁" in game.shop_volume_buy_button.text and "あと10回プレイで解禁" in game.shop_premium_buy_button.text)
	assert("レア10%・SR5%・新種0.001%" in game.shop_purchase_controls[3].text and "謎品種だけのたね" in game.shop_mystery_buy_button.text)
