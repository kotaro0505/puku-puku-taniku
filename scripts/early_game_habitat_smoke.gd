...
func _test_persistent_habitat(game:Node)->void:
	...
	assert(game.habitat_tutorial_complete and game.original_catalog_gifted and bool(game.unlocked_series.get("base",false)) and bool(game.discovered.get(tutorial_species,false)))
