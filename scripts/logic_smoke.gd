...
func _ready()->void:
	...
	assert(game.intro_panda_portrait.stretch_mode==TextureRect.STRETCH_KEEP_ASPECT_CENTERED and game.intro_dialog_panel.position in [Vector2(40,725),Vector2(40,385)] and "いいところでしょ" in game.intro_dialogue_label.text)
	...
	assert(game.current_mode=="habitat" and game.audio_manager.current_bgm_key=="habitat" and habitat_players<=1 and habitat_stream is AudioStreamOggVorbis and habitat_stream.loop and game.audio_manager.bgm_players[game.audio_manager.active_bgm].get_playback_position()<1.0);game._toggle_mode();assert(game.current_mode=="greenhouse" and game.audio_manager.current_bgm_key=="greenhouse");game._start_post_play_dialog("play2");assert(game.intro_panda_portrait.visible and (game.intro_panda_portrait.texture as AtlasTexture).atlas.resource_path=="res://assets/panda-clerk.png");game.intro_overlay.visible=false;game.shop_overlay.visible=false;game.tutorial_dialog_kind="";game._update_play_ui()
	...
	assert(game.catalog_species.size()==134 and game._series_species_entries("common").size()==10 and game._series_species_entries("base").size()==21 and game._series_species_entries("metal").size()==10 and game._series_species_entries("jewel").size()==10 and game._series_species_entries("jelly").size()==10 and game._series_species_entries("sweets").size()==10 and game._series_species_entries("gummy").size()==8 and game._series_species_entries("glow").size()==12 and game._series_species_entries("neon").size()==3 and game._series_species_entries("stone").size()==10 and game._series_species_entries("sea").size()==10 and game._series_species_entries("yumekawa").size()==10 and game._series_species_entries("forest_amber").size()==10 and game._mystery_event_species_ids().size()==6)
