...
func _ready()->void:
	...
	game._open_shop();game._on_shop_panda_tapped();var first_chatter:String=game.shop_chatter_label.text;assert(game.shop_chatter_bubble.visible and game.shop_chatter_portrait.visible and (game.shop_chatter_portrait.texture as AtlasTexture).atlas.resource_path=="res://assets/panda-clerk.png" and is_equal_approx(game.shop_chatter_bubble.position.x+game.shop_chatter_bubble.size.x*.5,288.0) and game.shop_chatter_bubble.position.y==276.0 and game.shop_chatter_bubble.size.y<=118.0 and not game.shop_chatter_action_button.visible);game._on_shop_panda_tapped();assert(game.shop_chatter_label.text!=first_chatter);var dismiss:=InputEventMouseButton.new();dismiss.button_index=MOUSE_BUTTON_LEFT;dismiss.pressed=true;game._on_shop_chatter_gui_input(dismiss);assert(not game.shop_chatter_bubble.visible)
