...
func _ready()->void:
	...
	game._reset_progression_state();game.bests={"colorata":100.0,"lutea":60.0,"shaviana":60.0};assert(not game._evaluate_best_spawn_unlocks());assert(not bool(game.greenhouse_available.get("hyalina_san_luis_de_la_paz",false)) and not bool(game.greenhouse_available.get("purpusorum",false)))
	...
	game.puku_points=1;var purchased_bags_before:int=game.normal_seed_bags;game._buy_seed_bag("normal");assert(game.puku_points==1 and game.normal_seed_bags==purchased_bags_before and "さやゲージ" in game.shop_message.text)
	assert(game.SEED_PACK_CONFIG.normal.count==12 and game.SEED_PACK_CONFIG.volume.count==36 and game.SEED_PACK_CONFIG.premium.count==24 and game.SEED_PACK_CONFIG.mystery.count==5);assert(is_equal_approx(float(game.SEED_PACK_CONFIG.normal.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.super),.05) and is_equal_approx(float(game.SEED_PACK_CONFIG.normal.new),.04) and is_equal_approx(game.NORMAL_SEED_UNLOCKED_NEW_RATE,.03) and is_equal_approx(game.NORMAL_SEED_LOCKED_NEW_RATE,.01) and is_equal_approx(game.NORMAL_SEED_NO_STAR_RATE,.81) and is_equal_approx(game.NORMAL_SEED_ONE_STAR_RATE,.10) and is_equal_approx(game.NORMAL_SEED_TWO_STAR_RATE,.05) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.new),.03) and is_equal_approx(float(game.SEED_PACK_CONFIG.premium.new),.03));assert(not game._seed_new_species_blocked("pinwheel") and not game._seed_new_species_blocked("tovarensis_tovar") and not game._seed_new_species_blocked("hyalina_san_luis_de_la_paz"))
	game.formal_play_count=13;game.volume_seed_unlocked=false;game.premium_seed_unlocked=false;game.volume_seed_bags=0;game.premium_seed_bags=0;game._refresh_seed_pack_unlocks();assert(not game._volume_seed_unlocked() and not game._premium_seed_unlocked());game.volume_seed_bags=1;game.premium_seed_bags=1;assert(game._volume_seed_unlocked() and game._premium_seed_unlocked())
	game.hidden_species_acquired.erase("strictiflora_bustamante");game.discovered.erase("strictiflora_bustamante");game.shop_visit_count=7;game.volume_seed_intro_seen=false;game.tutorial_steps.erase("volume_intro_step");game._prepare_shop_visit(false);assert(game.shop_visit_count==8 and not game.volume_seed_intro_seen and game.shop_chatter_sequence_kind!="volume_intro" and not bool(game.discovered.get("strictiflora_bustamante",false)));game._hide_shop_chatter(true)
	game.hidden_species_acquired.erase("tovarensis_tovar");game.discovered.erase("tovarensis_tovar");game.normal_play_count=5;game.tovar_next_play=6;game.tovar_attempt_count=0;game.active_seed_type="normal";game.play_active=true;game._prepare_tovar_event_for_play();assert(not game.tovar_event_active and game.plants.filter(func(p):return str(p.data.species_id)=="tovarensis_tovar").is_empty());game._resolve_tovar_event_after_play();assert(game.normal_play_count==6 and game.tovar_attempt_count==0 and not bool(game.discovered.get("tovarensis_tovar",false)));game.play_active=false
	...
	game._open_shop();game._hide_shop_chatter(true);game._on_shop_panda_tapped();var first_chatter:String=game.shop_chatter_label.text
	assert(game.shop_chatter_bubble.visible and game.shop_chatter_portrait.visible)
	assert((game.shop_chatter_portrait.texture as AtlasTexture).atlas.resource_path=="res://assets/panda-clerk.png" and not game.shop_chatter_action_button.visible)
	game._on_shop_panda_tapped();assert(not game.shop_chatter_bubble.visible and game.shop_chatter_label.text==first_chatter)
	game._on_shop_panda_tapped();assert(game.shop_chatter_bubble.visible and game.shop_chatter_label.text!=first_chatter)
	var dismiss:=InputEventMouseButton.new();dismiss.button_index=MOUSE_BUTTON_LEFT;dismiss.pressed=true;game._on_shop_chatter_gui_input(dismiss);assert(not game.shop_chatter_bubble.visible)
	game.puku_points=0;game.old_seed_bags=0;game.normal_seed_bags=0;game.volume_seed_bags=1;game.premium_seed_bags=0;game.mystery_seed_bags=0;assert(not game._shop_rescue_needed());game.volume_seed_bags=0;game.play_active=true;assert(not game._shop_rescue_needed());game.play_active=false;assert(game._shop_rescue_needed());game._on_shop_panda_tapped();assert(game.shop_chatter_action_button.visible and game.shop_chatter_bubble.position==Vector2(22,86) and "広告" not in game.shop_chatter_label.text);game._on_shop_background_gui_input(dismiss);assert(game.shop_chatter_bubble.visible);game.rescue_reward_in_progress=true;game._on_rescue_reward_ad_completed(false);assert(game.normal_seed_bags==0 and game.shop_chatter_bubble.visible and not game.rescue_reward_in_progress);game._request_rescue_reward_ad();await get_tree().create_timer(.8).timeout;assert(game.normal_seed_bags==1 and not game.rescue_reward_in_progress);game._on_rescue_reward_ad_completed(true);assert(game.normal_seed_bags==1);game._close_shop();game.puku_points=1
	...
	assert(game.plants.is_empty() and game.play_seed_animations_pending>=9 and game.play_seed_animations_pending<=12);assert(game.play_seeds_remaining==12-game.play_seed_animations_pending);assert(game.play_timer_label.text=="たね\n残り %d粒"%game.play_seeds_remaining);await get_tree().create_timer(.45).timeout
	assert(game.plants.size()>=9 and game.plants.size()<=12 and game.play_seed_animations_pending==0);assert(game.play_seeds_remaining==12-game.plants.size())
	...
	assert(is_equal_approx(game.SEED_POD_GAUGE_TARGET_CM,750.0) and is_equal_approx(game.PUKU_GAUGE_TARGET_CM,500.0))
	...
	assert(game.play_active and game.play_seeds_remaining<12-initial_count)
