...
func _ready()->void:
	...
	assert(game.shop_wallet_label.position.y==842.0 and game.shop_premium_buy_button.position==Vector2(48,945) and game.shop_premium_buy_button.size==Vector2(232,43) and game.armadillo_tap_button.position.y+game.armadillo_tap_button.size.y<836.0)
	...
	assert(game.SEED_PACK_CONFIG.normal.count==24 and game.SEED_PACK_CONFIG.volume.count==36 and game.SEED_PACK_CONFIG.premium.count==24 and game.SEED_PACK_CONFIG.mystery.count==5);assert(is_equal_approx(float(game.SEED_PACK_CONFIG.normal.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.super),.07) and is_equal_approx(float(game.SEED_PACK_CONFIG.premium.new),.01));assert(game._seed_new_species_blocked("pinwheel") and game._seed_new_species_blocked("tovarensis_tovar") and game._seed_new_species_blocked("hyalina_san_luis_de_la_paz")==false)
	game.formal_play_count=6;game.volume_seed_unlocked=false;game._refresh_seed_pack_unlocks();assert(not game._volume_seed_unlocked());game.formal_play_count=7;game._refresh_seed_pack_unlocks();assert(game._volume_seed_unlocked());game.formal_play_count=12;game.premium_seed_unlocked=false;game._refresh_seed_pack_unlocks();assert(not game._premium_seed_unlocked());game.formal_play_count=13;game._refresh_seed_pack_unlocks();assert(game._premium_seed_unlocked())
