...
func _ready()->void:
	...
	game._reset_progression_state();game.intro_story_complete=true;game._prepare_shop_visit(true);assert(game.shop_visit_count==0 and not game.armadillo_present);game.intro_story_complete=true;game.total_play_count=3;game.habitat_unlocked=true;game.encyclopedia_unlocked=true;game.buyback_unlocked=true;game.tutorial_steps["habitat_scroll_dialog"]=true;game.tutorial_steps["habitat_get_dialog"]=true;game.tutorial_steps["play1_dialog"]=true;game.intro_overlay.visible=false;game.shop_overlay.visible=false;game._update_play_ui()
	game._prepare_shop_visit(true);assert(game.shop_visit_count==1 and game.armadillo_present and game.armadillo_tap_button.visible);game._on_armadillo_tapped();assert(bool(game.hidden_species_acquired.get("pinwheel",false)) and bool(game.discovered.get("pinwheel",false)) and game.shop_chatter_acquired_species==["pinwheel"]);assert(game.shop_chatter_portrait.visible and game.shop_chatter_portrait.texture.resource_path=="res://assets/armadillo-dialogue.png" and game.shop_chatter_portrait.custom_minimum_size==Vector2(78,78) and game.shop_chatter_portrait.stretch_mode==TextureRect.STRETCH_KEEP_ASPECT_CENTERED);var hidden_count:int=game.hidden_species_acquired.size();game._on_armadillo_tapped();assert(game.hidden_species_acquired.size()==hidden_count and game.shop_chatter_acquired_species.is_empty());game._hide_shop_chatter(true)
	...
	assert(game.shop_wallet_label.position.y==836.0 and game.shop_premium_buy_button.position==Vector2(48,939) and game.shop_premium_buy_button.size==Vector2(232,53) and game.armadillo_tap_button.position.y+game.armadillo_tap_button.size.y<832.0)
	game.coins=1000;var purchased_bags_before:int=game.normal_seed_bags;game._buy_seed_bag("normal");assert(game.coins==500 and game.normal_seed_bags==purchased_bags_before+1);game.coins=499;var failed_bags_before:int=game.normal_seed_bags;game._buy_seed_bag("normal");assert(game.coins==499 and game.normal_seed_bags==failed_bags_before);game.coins=1000
	assert(game.SEED_PACK_CONFIG.normal.count==24 and game.SEED_PACK_CONFIG.volume.count==36 and game.SEED_PACK_CONFIG.premium.count==24 and game.SEED_PACK_CONFIG.mystery.count==5);assert(is_equal_approx(float(game.SEED_PACK_CONFIG.normal.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.super),.05) and is_equal_approx(float(game.SEED_PACK_CONFIG.premium.new),.01));assert(game._seed_new_species_blocked("pinwheel") and game._seed_new_species_blocked("tovarensis_tovar") and game._seed_new_species_blocked("hyalina_san_luis_de_la_paz")==false)
	game.formal_play_count=3;game.volume_seed_unlocked=false;game.premium_seed_unlocked=false;game._update_shop_ui();assert("あと4回プレイで解禁" in game.shop_volume_buy_button.text and "あと10回プレイで解禁" in game.shop_premium_buy_button.text and game.shop_bag_label.text=="たね袋を1袋ずつ購入できます");game.formal_play_count=6;game._refresh_seed_pack_unlocks();assert(not game._volume_seed_unlocked());game.formal_play_count=7;game._refresh_seed_pack_unlocks();assert(game._volume_seed_unlocked());game.formal_play_count=12;game.premium_seed_unlocked=false;game._refresh_seed_pack_unlocks();assert(not game._premium_seed_unlocked());game.formal_play_count=13;game._refresh_seed_pack_unlocks();assert(game._premium_seed_unlocked())
	game.hidden_species_acquired.erase("strictiflora_bustamante");game.discovered.erase("strictiflora_bustamante");game.shop_visit_count=7;game.volume_seed_intro_seen=false;game._prepare_shop_visit(false);assert(game.shop_visit_count==8 and game.volume_seed_intro_seen and not bool(game.hidden_species_acquired.get("strictiflora_bustamante",false)));game._hide_shop_chatter(true);game._prepare_shop_visit(false);assert(game.shop_visit_count==9 and bool(game.hidden_species_acquired.get("strictiflora_bustamante",false)) and bool(game.discovered.get("strictiflora_bustamante",false)));game._hide_shop_chatter(true)
	...
	assert(game.catalog_species.size()==15)
	...
	for required in ["laui","golden_laui","colorata","affinis","lutea","hyalina_san_luis_de_la_paz","purpusorum","shaviana","kannte","pinwheel","tovarensis_tovar","transparent_succulent","strictiflora_bustamante"]:assert(required in species_ids)
