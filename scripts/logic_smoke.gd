...
func _ready()->void:
	...
	assert(game.SEED_PACK_CONFIG.normal.count==12 and game.SEED_PACK_CONFIG.volume.count==36 and game.SEED_PACK_CONFIG.premium.count==24 and game.SEED_PACK_CONFIG.mystery.count==5);assert(is_equal_approx(float(game.SEED_PACK_CONFIG.normal.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.super),.05) and is_equal_approx(float(game.SEED_PACK_CONFIG.normal.new),.04) and is_equal_approx(game.NORMAL_SEED_UNLOCKED_NEW_RATE,.03) and is_equal_approx(game.NORMAL_SEED_LOCKED_NEW_RATE,.01) and is_equal_approx(game.NORMAL_SEED_NO_STAR_RATE,.81) and is_equal_approx(game.NORMAL_SEED_ONE_STAR_RATE,.10) and is_equal_approx(game.NORMAL_SEED_TWO_STAR_RATE,.05) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.new),.03) and is_equal_approx(float(game.SEED_PACK_CONFIG.premium.new),.03));assert(game._seed_new_species_blocked("pinwheel") and game._seed_new_species_blocked("tovarensis_tovar") and game._seed_new_species_blocked("hyalina_san_luis_de_la_paz")==false)
	game.formal_play_count=2;game.volume_seed_unlocked=false;game.premium_seed_unlocked=false;game._refresh_seed_pack_unlocks();assert(not game._volume_seed_unlocked() and not game._premium_seed_unlocked());game.formal_play_count=3;game._refresh_seed_pack_unlocks();assert(game._volume_seed_unlocked());game.formal_play_count=12;game.premium_seed_unlocked=false;game._refresh_seed_pack_unlocks();assert(not game._premium_seed_unlocked());game.formal_play_count=13;game._refresh_seed_pack_unlocks();assert(game._premium_seed_unlocked())
	game.hidden_species_acquired.erase("strictiflora_bustamante");game.discovered.erase("strictiflora_bustamante");game.shop_visit_count=7;game.volume_seed_intro_seen=false;game.tutorial_steps.erase("volume_intro_step");game._prepare_shop_visit(false);assert(game.shop_visit_count==8 and not game.volume_seed_intro_seen and game.shop_chatter_sequence_kind!="volume_intro" and bool(game.hidden_species_acquired.get("strictiflora_bustamante",false)) and bool(game.discovered.get("strictiflora_bustamante",false)));game._hide_shop_chatter(true);await _drain_species_get(game)
	...
	assert(game.plants.is_empty() and game.play_seed_animations_pending>=9 and game.play_seed_animations_pending<=12);assert(game.play_seeds_remaining==12-game.play_seed_animations_pending);assert(game.play_timer_label.text=="たね袋\n残り %d粒"%game.play_seeds_remaining);await get_tree().create_timer(.45).timeout
