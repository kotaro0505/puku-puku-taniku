...
func _ready()->void:
	...
	game._prepare_shop_visit(true);assert(game.shop_visit_count==1 and game.armadillo_present and game.armadillo_tap_button.visible);game._on_armadillo_tapped();assert(bool(game.hidden_species_acquired.get("pinwheel",false)) and bool(game.discovered.get("pinwheel",false)));assert(game.shop_chatter_portrait.visible and (game.shop_chatter_portrait.texture as AtlasTexture).atlas.resource_path=="res://assets/shop-background-armadillo.jpg");var hidden_count:int=game.hidden_species_acquired.size();game._on_armadillo_tapped();assert(game.hidden_species_acquired.size()==hidden_count);game._hide_shop_chatter(true)
