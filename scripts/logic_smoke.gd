...
func _ready()->void:
	...
	game._prepare_shop_visit(true);assert(game.shop_visit_count==1 and game.armadillo_present and game.armadillo_tap_button.visible);game._on_armadillo_tapped();assert(bool(game.hidden_species_acquired.get("pinwheel",false)) and bool(game.discovered.get("pinwheel",false)));var armadillo_portrait:=game.shop_chatter_portrait.texture as AtlasTexture;assert(game.shop_chatter_portrait.visible and armadillo_portrait.atlas.resource_path=="res://assets/armadillo-dialogue.png" and armadillo_portrait.region==Rect2(60.0,0.0,840.0,860.0));var hidden_count:int=game.hidden_species_acquired.size();game._on_armadillo_tapped();assert(game.hidden_species_acquired.size()==hidden_count);game._hide_shop_chatter(true)
