...
func _ready()->void:
	...
	assert(game.SEED_PACK_CONFIG.normal.count==24 and game.SEED_PACK_CONFIG.volume.count==36 and game.SEED_PACK_CONFIG.premium.count==24 and game.SEED_PACK_CONFIG.mystery.count==5);assert(is_equal_approx(float(game.SEED_PACK_CONFIG.normal.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.rare),.10) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.super),.05) and is_equal_approx(float(game.SEED_PACK_CONFIG.normal.new),.05) and is_equal_approx(game.NORMAL_SEED_UNLOCKED_NEW_RATE,.04) and is_equal_approx(game.NORMAL_SEED_LOCKED_NEW_RATE,.01) and is_equal_approx(float(game.SEED_PACK_CONFIG.volume.new),.03) and is_equal_approx(float(game.SEED_PACK_CONFIG.premium.new),.03));assert(game._seed_new_species_blocked("pinwheel") and game._seed_new_species_blocked("tovarensis_tovar") and game._seed_new_species_blocked("hyalina_san_luis_de_la_paz")==false)
