...
func _ready() -> void:
	...
	assert(bool(game.unlocked_series.get("base", false)) and game.encyclopedia_overlay.visible)
	...
	assert(Localizer.species_name("ja", game._catalog_entry("lutea")) in trio_text)
	...
	assert(bool(game.discovered.get("lutea", false)))
	assert(bool(game.discovered.get("shaviana", false)))
	assert(game.main_story_stage == game.StoryProgressionClass.STAGE_FIND_HABITAT)

	print("FIRST_PLAY_TUTORIAL_SMOKE_OK seed=1 plant=1 species=colorata safe=true discovery=major trio=colorata+lutea+shaviana")
	get_tree().quit()
