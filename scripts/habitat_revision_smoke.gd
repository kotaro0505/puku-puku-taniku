...
func _test_growth_and_jelly()->void:
	...
	assert(is_equal_approx(float(normal.diameter_cm),expected) and not bool(normal.jellied) and is_zero_approx(float(normal.jelly_hazard_accumulated)))
	...
	assert(str(crossing.individual_id) in crossing_events.ready and float(crossing.harvest_ready_reached_unix)>now-2000.0 and float(crossing.harvest_ready_reached_unix)<now)
	...
	var jelly_plants:Array[Dictionary]=[jelly_fast,jelly_slow];HabitatWildSystemClass.advance_time(jelly_plants,now)
	assert(bool(jelly_fast.jellied) and bool(jelly_slow.jellied))
	assert(float(jelly_fast.jellied_unix)<float(jelly_slow.jellied_unix))
...
func _test_beacon_and_debug_tools(game:Node)->void:
	...
	game._install_panda_beacon(str(target.individual_id));assert(bool(target.panda_beacon_installed) and game._panda_beacon_used_count()==1)
	assert(game.habitat_notification_service.scheduled_snapshot().has(str(target.individual_id)))
	game._remove_panda_beacon(str(target.individual_id));assert(not bool(target.panda_beacon_installed) and game._panda_beacon_used_count()==0)
	...
	game._install_panda_beacon(str(target.individual_id));assert(game.habitat_time_multiplier==86400)
	game._debug_jump_habitat_time(86400);assert(float(target.diameter_cm)>=30.0 and not bool(target.jellied) and bool(target.panda_beacon_installed))
	assert(game.habitat_debug_log.any(func(line):return "30cm" in str(line)))
	game._build_habitat_items(true);var target_item:Dictionary=game.habitat_pickups.filter(func(item):return str(item.get("individual_id",""))==str(target.individual_id))[0];game._collect_habitat_wild_plant(target_item)
	assert(game._panda_beacon_used_count()==0)
	var jelly_target:Dictionary=game.habitat_wild_plants[0];jelly_target.diameter_cm=29.9999;jelly_target.jellied=false;jelly_target.jelly_hazard_accumulated=0.0;jelly_target.jelly_threshold=HabitatWildSystemClass.jelly_hazard_rate_per_second();jelly_target.last_updated_unix=Time.get_unix_time_from_system();game._refresh_habitat_growth_profile(jelly_target)
	game._install_panda_beacon(str(jelly_target.individual_id));assert(game._panda_beacon_used_count()==1)
	game._debug_jump_habitat_time(60);assert(bool(jelly_target.jellied) and not bool(jelly_target.panda_beacon_installed) and game._panda_beacon_used_count()==0)
	var owned_before:int=game.panda_beacon_count;var unlocked_before:bool=game.panda_beacon_unlocked;var old_ids:Array[String]=game._habitat_individual_ids();game._debug_reset_normal_habitat()
	assert(game.panda_beacon_count==owned_before and game.panda_beacon_unlocked==unlocked_before and game._panda_beacon_used_count()==0 and game._habitat_individual_ids()!=old_ids)
...
func _test_save_persistence(game:Node)->void:
	...
	game.panda_beacon_unlocked=true;game.panda_beacon_count=3
	...
	game.habitat_wild_plants.clear();game.habitat_wild_initialized=false;game.habitat_wild_next_spawn_unix=0;game.panda_beacon_unlocked=false;game.panda_beacon_count=0;game._load_save()
