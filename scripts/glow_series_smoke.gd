...
func _ready()->void:
	...
	assert(game._can_browse_series(glow) and not game._is_series_unlocked(glow) and not game._catalog_purchase_enabled(glow))
	...
	for entry in entries:
		...
		assert(image_path.begins_with("res://assets/plants/glow/") and image_path.ends_with(".png") and ResourceLoader.exists(image_path))
	...
	for image in game.encyclopedia_card_images:
		...
		if image.texture!=null:assert(str(image.texture.resource_path).begins_with("res://assets/plants/glow/"))
	print("GLOW_SERIES_SMOKE_OK species=",entries.size()," images=",seen.size()," silhouette_cards=",game.encyclopedia_grid.get_child_count()," seed_pool=0 purchase=false")
	get_tree().quit()
